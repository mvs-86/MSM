...
// sigma_gm = sigma * g.m; lev = GJR asymmetry coefficient (gamma >= 0).
// State-conditional variance: sigma_gm(m)^2 + lev * r_{t-1}^2 * I(r_{t-1}<0).
// [[Rcpp::export]]
double Amsm_gjr_ll_fast_cpp(const arma::vec& dat, const arma::rowvec& sigma_gm,
	...
	pi_t.fill(1.0 / k);
	for (int i = 0; i < N; i++) {
		double neg_sq = (i > 0 && dat(i - 1) < 0.0) ? dat(i - 1) * dat(i - 1) : 0.0;
		arma::rowvec sgm = arma::sqrt(arma::square(sigma_gm) + lev * neg_sq);
		piA     = pi_t * A;
...
// Scalar LL for optimization, kbar >= 8.
// [[Rcpp::export]]
double Amsm_gjr_ll_kron_cpp(const arma::vec& dat, const arma::rowvec& sigma_gm,
...
// [[Rcpp::export]]
List Amsm_gjr_fast_cpp(const arma::vec& dat, const arma::rowvec& sigma_gm,
                        const arma::mat& A, double lev) {
	int N = dat.n_rows, k = sigma_gm.n_elem;
	const double inv_sqrt2pi = 1.0 / std::sqrt(2.0 * M_PI);
	double pidenom, ll = 0.0;
...
// Full output (filtered + LL + LLs), kbar >= 8.
// [[Rcpp::export]]
List Amsm_gjr_kron_cpp(const arma::vec& dat, const arma::rowvec& sigma_gm,
                        double b, double gamma_kbar, int kbar, double lev) {
	int N = dat.n_rows, k = sigma_gm.n_elem;
	const double inv_sqrt2pi = 1.0 / std::sqrt(2.0 * M_PI);
	double pidenom, ll = 0.0;
