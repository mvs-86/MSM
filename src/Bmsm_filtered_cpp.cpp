...
// In-place Kronecker-factored pi*A for Bmsm.
// A = A_{kbar-1} ⊗ ... ⊗ A_1 ⊗ A_0 built via kron(a, A), so A_i has stride = 4^i.
// Each A_i = bmsm_transition(gamma_k[i], lamda, rho_m), a symmetric 4x4 matrix.
// Exploits T's structure: row3=rev(row0), row2=rev(row1) → 12 mults per quad.
...
// [[Rcpp::export]]
List Bmsm_filtered_kron(const arma::mat& dat, const arma::mat& gm,
