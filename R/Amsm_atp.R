#' Asymmetric MSM with asymmetric transition probabilities.
#'
#' Estimates univariate MSM(k) with ATP leverage: two transition matrices,
#' A_pos (r_{t-1}>=0) and A_neg built from gamma_k*(1+lev) (r_{t-1}<0).
#' Positive lev accelerates volatility switching after negative returns.
#'
...
#'
#' @return an \code{amsmmodel} object with type "atp".
#'
#' @export
Amsm_atp <- function(ret, kbar = 1, n.vol = 252, para0 = NULL, nw.lag = 0) {
  ...
  msm.fit <- nlminb(x0, Amsm_atp_ll, lower = lb, upper = ub,
                    kbar = kbar, dat = ret, n.vol = n.vol,
                    control = list(eval.max = 500, iter.max = 300))

  msm.estimate <- Amsm_atp_likelihood(msm.fit$par, kbar, ret, n.vol)
  se           <- Amsm_atp_std_err(msm.fit$par, kbar, ret, n.vol, nw.lag)
  ...
  msm.estimate$type              <- "atp"

  class(msm.estimate) <- "amsmmodel"
  msm.estimate
}

# Objective function for nlminb. A_neg built inside from gammak*(1+lev).
Amsm_atp_ll <- function(para, kbar, dat, n.vol) {
  m0     <- para[1]
  b      <- para[2]
  gama.k <- para[3]
  ...
  if (kbar >= 8) {
    LL <- Amsm_atp_ll_kron_cpp(dat, sigma_gm, b, gama.k, kbar, lev)
  } else {
    A_pos <- Msm_A(b, gama.k, kbar)
    A_neg <- Msm_A(b, min(gama.k * (1 + lev), 0.9999), kbar)
    LL <- Amsm_atp_ll_fast_cpp(dat, sigma_gm, A_pos, A_neg)
  }
...
# Full output: filtered probabilities + LL + LLs. Stores A_pos as $A.
#' @export
Amsm_atp_likelihood <- function(para, kbar, dat, n.vol) {
  m0     <- para[1]
  b      <- para[2]
  gama.k <- para[3]
  sigma  <- para[4] / sqrt(n.vol)
  ...
  if (kbar >= 8) {
    likelihood <- Amsm_atp_kron_cpp(dat, sigma_gm, b, gama.k, kbar, lev)
  } else {
    likelihood <- Amsm_atp_fast_cpp(dat, sigma_gm, A_pos, A_neg)
  }

  likelihood$A   <- A_pos   # A_pos stored for predict/smooth (symmetric forecast base)
  likelihood$g.m <- g.m
  likelihood
}

Amsm_atp_grad <- function(para, kbar, ret, n.vol) {
  ...
  res <- parallel::mclapply(para_list, Amsm_atp_likelihood,
                            kbar = kbar, dat = ret, n.vol = n.vol,
...
Amsm_atp_hessian <- function(para, kbar, ret, n.vol) {
  check_para <- function(x) {
    if (x[1] >= 2) x[1] <- 1.9999
    if (x[3] >= 1) x[3] <- 0.9999
    x
  ...
  para.size <- length(para)
  para      <- as.matrix(para)
  f.ll      <- Amsm_atp_likelihood(check_para(para), kbar, ret, n.vol)$LL
  ...
  res <- parallel::mclapply(para_list,
                            function(p) Amsm_atp_likelihood(p, kbar, ret, n.vol)$LL,
                            mc.cores = getOption("mc.cores", 1L))
  res <- unlist(res)
...
#' @export
Amsm_atp_std_err <- function(para, kbar, ret, n.vol, lag = 0) {
  if (kbar == 1) {
    grad <- Amsm_atp_grad(para, kbar, ret, n.vol)
    grad <- grad[, -2]
    J    <- t(grad) %*% grad
    s    <- sqrt(diag(solve(J)))
    se   <- matrix(c(s[1], NA, s[2:4]), ncol = 1)
  } else {
    H  <- Amsm_atp_hessian(para, kbar, ret, n.vol)
